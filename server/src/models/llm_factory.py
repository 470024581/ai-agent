...
class LLMFactory:
    ...
    @classmethod
    def get_reasoning_llm(cls) -> BaseLanguageModel:
        """Get Reasoning LLM instance using reasoning_model when available."""
        ai_config = config.get_ai_config()
        # Clone config with reasoning model if provided
        cfg = dict(ai_config)
        reasoning_model = ai_config.get("reasoning_model")
        if reasoning_model:
            cfg["model"] = reasoning_model
        # Force recreate if model differs
        return cls._create_llm(cfg)

    @classmethod
    def get_llm(cls) -> BaseLanguageModel:
        """Get LLM instance (chat model by default), create if not exists"""
        ai_config = config.get_ai_config()
...
def get_chat_llm() -> BaseLanguageModel:
    """Alias for chat model (same as get_llm)."""
    return LLMFactory.get_llm()

def get_reasoning_llm() -> BaseLanguageModel:
    """Get global Reasoning LLM instance"""
    return LLMFactory.get_reasoning_llm()
